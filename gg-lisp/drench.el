...
(defun drench-flood-fill (value)
  (cl-loop with tovisit = '((0 0))
	   with visited = '()
	   with similar = (drench-get-square 0 0)
	   until (null tovisit)
	   do (let* ((pos (pop tovisit))
		     (x (car pos))
		     (y (cadr pos)))
		(drench-set-square x y value)
		(push pos visited)
		(cl-loop for (newx newy) in (list (list (+ x 1) y)
						  (list (- x 1) y)
						  (list x (+ y 1))
						  (list x (- y 1)))
			 when (and (not (member (list newx newy) visited))
				   (<= 0 newx)
				   (< newx *drench-board-size*)
				   (<= 0 newy)
				   (< newy *drench-board-size*)
